  ...
  try {
    ...
    await page.route('http://coffee.test/**', async r => {
      ...
      if (pathname === '/api/v1/nearby') {
        ...
        const candidates = empty ? [] : matching;
        ...
        return r.fulfill({json: {cafes: candidates.slice(offset, offset + limit), total: candidates.length,
          distance_mode: body.distance_mode, candidate_count: candidates.length, candidate_limit: null, unreachable_count: 0}});
      }
    ...
    assert.equal(await page.evaluate(() => [...cafeMarkers.values()][0].options.title), '11. Café 11');
    assert.equal(await page.evaluate(() => cafeMarkers.size), 10);
    ...
    assert.equal(await page.evaluate(() => cafeMarkers.size), 5);
    ...
    await page.getByText('1–10 of 45', {exact: true}).waitFor();
    assert.match(await page.locator('#search-scope').innerText(), /checked all 45/);
    assert.equal(await page.locator('#result-count').innerText(), '45 cafés ranked by road');
